...
OUT = Path("outputs/problem_plane_geometry_refactor_v4.json")
...
def _params_for(spec: Dict[str, Any], posture_name: str, descriptor: Dict[str, float], pred: List[str], pred_scores: Dict[str, float]) -> Dict[str, Any]:
    ...
    params["descriptor_hypothesis"] = {
        ...
        "source": "problem_plane_geometry_refactor_v4",
    }
...
def _run_bandit(spec: Dict[str, Any], posture_name: str, descriptor: Dict[str, float], pred: List[str], pred_scores: Dict[str, float], seed: int) -> Dict[str, Any]:
    core = build_validated_co_core(_params_for(spec, posture_name, descriptor, pred, pred_scores), study_name="problem_plane_geometry_refactor_v4", manifest_path=DEFAULT_CANONICAL_MANIFEST, agent_name=DEFAULT_CANONICAL_AGENT_NAME)
    ...
    for t in range(BANDIT_HORIZON):
        ...
        if done:
            break
    assert_valid_co_rollout(study_name="problem_plane_geometry_refactor_v4", signal_bus_votes=votes, co_policies=policies)
    return {
        "final_regret": regret,
...
def main() -> None:
    out = {"study": "problem_plane_geometry_refactor_v4", "bandit": {}, "renewal": {}}
    for spec in BANDITS:
