...
def posture_scores(descriptor: Dict[str, float], *, target_scope: str = "mixed") -> Dict[str, float]:
    ...
    if target_scope == "hypothesis_over_anchor" and deform <= 0.15:
        # Generic static-hypothesis regime law:
        # - if ambiguity is high and rival-testing budget is weak, balanced should beat the extremes
        # - if discriminability and coverage are both strong, sharper commitment becomes legitimate
        # - reopen should not dominate purely static regimes unless deformation/revision really warrants it
        ambiguity_pressure = (1.0 - disc) * (1.0 - coverage)
        commitment_readiness = disc * coverage * (1.0 - revision)
        sharp = 0.58 * commitment_readiness + 0.22 * disc + 0.12 * coverage + 0.08 * (1.0 - revision)
        balanced = 0.52 * ambiguity_pressure + 0.20 * revision + 0.18 * (1.0 - deform) + 0.10 * (1.0 - abs(disc - coverage))
        reopen = 0.40 * revision * deform + 0.24 * ambiguity_pressure + 0.20 * (1.0 - coverage) * deform + 0.16 * (1.0 - disc) * deform
        return {
    ...
    # Generic fallback for mixed/emergent regimes.
    sharp_pull = 0.34 * disc + 0.28 * persist + 0.22 * coverage + 0.16 * (1.0 - revision)
    reopen_pull = 0.34 * revision + 0.30 * deform + 0.20 * (1.0 - coverage) + 0.16 * (1.0 - disc)
    balanced_pull = 0.55 * (1.0 - abs(sharp_pull - reopen_pull)) + 0.25 * (1.0 - deform) + 0.20 * coverage
    return {
