...
class HeaderSSI(BaseHeader):
    ...
    def apply_static(self) -> None:
        ...
        self.state.thinness = float(self.st.get("thinness_prior", 0.25))

        self.st["co_base"] = float(self.st.get("co_base", 0.70))
        self.st["anneal_beta"] = float(self.st.get("anneal_beta", 0.010))
