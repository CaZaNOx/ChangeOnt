...
def _core(family: str, *, reduction_mode: str | None = None, axis_override: Dict[str, float] | None = None):
    cfg={
        "name": f"reduction_cmp_v4_{family}_{reduction_mode or '10a'}",
    ...
    for label, probs in [("bandit_easy", [0.85,0.55,0.25,0.10]), ("bandit_hard", [0.55,0.50,0.45,0.40])]:
        for seed in range(3):
            ...
            for t in range(10):
                ...
                if d: break
    # renewal broader
    ren_variants=[('renewal_stable_clean',0.0,0.02),('renewal_stable_noisy',0.2,0.02),('renewal_volatile_clean',0.0,0.12),('renewal_volatile_noisy',0.2,0.12)]
    for label, noise, p_ren in ren_variants:
        for seed in range(3):
            env=CodebookRenewalEnvW(EnvCfg(A=8,L_win=6,p_ren=p_ren,p_noise=noise,T_max=36), seed=seed)
    ...
    for mode_name, reduction_mode in modes.items():
        ...
        for vname, probs in variants.items():
            ...
            for sname, sval in shapes.items():
                trials=[]
                for seed in range(3):
                    env=BernoulliBanditEnv(probs,horizon=40); env.reset(seed=seed)
...
def main() -> Dict[str, Any]:
    ...
    return {
        'study':'reduced_axis_harder_comparison_v4',
