...
The terms “subject,” “awareness,” and “experience” are not independently reducible by reflective analysis at this stage. Each only acquires its sense in immediate, lived reference to the others. Their meaning is not discovered by logical decomposition, but by the impossibility of denying that “I, as reader, am aware of experience.”  
...
> - **High blurriness:** Many possible alternatives for CoE(y) remain plausible (i.e., the subject’s experience is distributed or “smeared” across options, with no single one being certain).
...
> - **Blurriness(E(x,y)) =** The _probability distribution_ over Y$_i$ as assigned by the subject’s SoE(x).
...
We formulate
[[DF 2.1_3.]]  
diff(x,y) := difference of happening x and happening y. Were Diff(x,y)=∅ expresses that there is no difference 
and
diff(x,y) ≠ ∅ expresses that there is difference.
...
Note the RDS is not aware of the pointer ⭦(x). This is a tool for the human reader to label the result of poiniting.
...
But first we need to define an updated way of writing pointer ⭦(x) 

with definition 
[[DF 2.1_7.]]  Pointer ⭦(x) := implied difference in a happening. Where x refers to what is pointed to.
...
We say
$\exists x: (x⇖y)$ this implies that $\exists y: (x⇖y)$ 
...
Prior(y) := The x that is pointed at by y. (x⇖y).  
...
↝ ∿
[[DF 3.1_3.]]  
structurallyPointsTo (⇘):=
"x⇘y" means: x is a structural prior of y that, by virtue of being pointed at, enables the arisal of y as different.
This is not a causal or temporal arrow, but a structural precondition for newness at y.
(x⇘y):= x points to y as implied by the structural ability of pointing in y. Put differently we say $\exists y: (Arisal(y))$ and $Arisal(y)\implies \exists x:(Prior(x))$
...
Since the DS points to something different because arisal implies that what arose wasn't the same we can state that $\exists x,y: (x⇘y)$ where diff(x,y)$\neq \emptyset$ 

if diff(x,y)=$\emptyset$ for (x⇘y) then x=y and Arisal(y)$\not\implies$ x since y didn't arise from itself.

To summarize we can say that the ES experiences y as C(now) where C(now)$\implies\exists x,y: (x⇖y)$ and diff(x,y) $\neq\emptyset$ where y=now and x=$\lnot now$ were (x⇘y) is understood as Prior(y)⇘Arisal(y)
...
11) $\exists ⇘:$($\lnot$Now ⇘ Now)
12) $\exists x,y:$  (x⇘ y)$\land (x=Prior(y))\land (y=Arisal(y)) \land (y=New(y))$ $\implies$ (Prior(y)⇘(Arisal(y)$\land$New(y)))
13)  $\exists x: x\notin$ Now$\land$ x$\in\lnot$Now
14) $\exists y: y\in$ Now$\land$ y$\notin\lnot$Now
15) $\exists x,y:$ x$\notin$Now $\land$ y$\in$ Now$\implies$ diff(x,y) $\neq\emptyset$
16) $\exists x,y:$⇘(x, y) where diff(x,y)$\neq\emptyset$

P2: We assume that there is a x=Prior(y)  with no change at all in x we say $\lnot$C(x). Or in other words x is in pure stasis. 

D1:
Something being in pure stasis can be expressed as $\lnot$C(x) and is not pointing to anything different. Or if formulated  differently if there was a pointer in $\lnot$C(x) it could only point to itself/something where diff(x,y)=$\emptyset$  meaning x=y therefor the only possible pointer would be ⇘(x, x). 
If there would be $\exists x,y:$ (x⇘y) with diff(x,y)$\neq\emptyset$ that would mean that x would lead to $\lnot$x. Meaning x would change to something else. This would therefore imply that C(x) is the case.
...
D2
x is understood as that which was pointed at by the DS. This is expressing the structural fact that x is Prior of y where C(y) is the case. Meaning there is (x⇘y) where diff(x,y)$\neq\emptyset$ (as seen in P1)
P2 states that x is a Prior of y
Co1
Contradiction ⊥: D1$\land$D2
