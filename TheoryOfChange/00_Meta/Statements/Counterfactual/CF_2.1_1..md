...
So if we would state that Chance is static we would say that Change means that nothing happens but we defined change in DF0.6 as that what happens. So if there was no happening the term change wouldn't be meaningful since there is nothing that happens and therefore change as defined in DF0.6 would never be the case.
