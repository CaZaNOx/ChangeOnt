...
```
Returns a dict mapping quotient edges (cls_u, cls_v) to their infimum base cost.
"""
lifted: DefaultDict[Edge, float] = defaultdict(lambda: math.inf)
for (u, v), c in base_costs.items():
    cu, cv = reps.get(u, u), reps.get(v, v)
    key = (cu, cv)
    if c < lifted[key]:
        lifted[key] = c
return dict(lifted)