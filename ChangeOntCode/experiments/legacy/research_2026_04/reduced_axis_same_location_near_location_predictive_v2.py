...
def _core(family: str, *, reduction_mode: str | None = None, axis_override: Dict[str, float] | None = None):
    cfg={
        "name": f"reduction_cmp_v2_{family}_{reduction_mode or '10a'}",
...
def _sample_states() -> List[Dict[str, Any]]:
    ...
    for label, probs in [("bandit_easy", [0.85,0.55,0.25,0.10]), ("bandit_hard", [0.55,0.50,0.45,0.40])]:
        for seed in range(2):
            env=BernoulliBanditEnv(probs,horizon=24); env.reset(seed=seed)
            ...
            for t in range(6):
                ...
                if packet:
                    ...
                    out.append({'family':'bandit','variant':label,'seed':seed,'t':t,'packet':packet,'axes10':bundle['effective_axes'],'axes7':project_full_to_reduced7(bundle['effective_axes'])})
                a=sel.get('action',0)
                _,r,d,_=env.step(a); ag.update({'action':a,'reward':float(r),'done':bool(d)})
                if d: break
    # renewal
    for label, noise, p_ren in [("renewal_stable_noisy",0.05,0.02), ("renewal_volatile_noisy",0.20,0.12)]:
        for seed in range(2):
            env=CodebookRenewalEnvW(EnvCfg(A=4,L_win=6,p_ren=p_ren,p_noise=noise,T_max=24), seed=seed)
            ...
            while not done and t < 10:
                ...
                if packet:
                    bundle=_bundle(core, packet)
                    out.append({'family':'renewal','variant':label,'seed':seed,'t':t,'packet':packet,'axes10':bundle['effective_axes'],'axes7':project_full_to_reduced7(bundle['effective_axes'])})
                a=sel.get('action',0)
                obs,r,done,_=env.step(a); ag.update({'action':a,'reward':float(r),'done':bool(done),'obs':obs,'A':4}); t += 1
    # maze
    for label, spec_fn in [("maze_visible_static", lambda s: MazeSpec(width=7,height=7,seed=s,partial_observability=False,dynamic_walls=False)), ("maze_partial_dyn", lambda s: MazeSpec(width=7,height=7,seed=s,partial_observability=True,view_radius=1,dynamic_walls=True,wall_flip_prob=0.08,max_flips_per_step=1))]:
        for seed in range(2):
            ...
            for t in range(6):
                ...
                if packet:
                    bundle=_bundle(core, packet)
                    out.append({'family':'maze','variant':label,'seed':seed,'t':t,'packet':packet,'axes10':bundle['effective_axes'],'axes7':project_full_to_reduced7(bundle['effective_axes'])})
...
def near_location_compare(states: List[Dict[str, Any]]) -> Dict[str, Any]:
    fams = ['bandit','renewal','maze']
    pairs = [('bandit','renewal'),('bandit','maze'),('renewal','maze')]
    res={}
    for label, key in [('full10','axes10'),('reduced7','axes7')]:
        best = {}
        for a,b in pairs:
            bestd = None; bestpair = None
            A = [s for s in states if s['family']==a]
            B = [s for s in states if s['family']==b]
            for sa in A:
                for sb in B:
                    d = _l2(sa[key], sb[key])
                    if bestd is None or d < bestd:
                        bestd = d; bestpair=(sa,sb)
            best[f'{a}_{b}']={
                'distance': bestd,
                'a_variant': bestpair[0]['variant'], 'a_seed': bestpair[0]['seed'], 'a_t': bestpair[0]['t'],
                'b_variant': bestpair[1]['variant'], 'b_seed': bestpair[1]['seed'], 'b_t': bestpair[1]['t'],
            }
        res[label]=best
    return res


def bandit_predictive_compare() -> Dict[str, Any]:
...
def renewal_predictive_compare() -> Dict[str, Any]:
    variants={
        'stable_clean': {'noise':0.05,'p_ren':0.02},
        'stable_noisy': {'noise':0.20,'p_ren':0.02},
        'volatile_clean': {'noise':0.05,'p_ren':0.12},
        'volatile_noisy': {'noise':0.20,'p_ren':0.12},
    }
    modes={"full10":None,"reduced7":"investigatory_7a_v1"}
    ...
    for mode_name, reduction_mode in modes.items():
        ...
        for vname, cfg in variants.items():
            ...
            for sname, sval in shapes.items():
                ...
                trials=[]
                for seed in range(2):
                    env=CodebookRenewalEnvW(EnvCfg(A=8,L_win=6,p_ren=cfg['p_ren'],p_noise=cfg['noise'],T_max=40), seed=seed)
...
def maze_predictive_compare() -> Dict[str, Any]:
    variants={
        'visible_static': lambda s: MazeSpec(width=7,height=7,seed=s,partial_observability=False,dynamic_walls=False),
        'partial_dynamic': lambda s: MazeSpec(width=7,height=7,seed=s,partial_observability=True,view_radius=1,dynamic_walls=True,wall_flip_prob=0.08,max_flips_per_step=1),
    }
    modes={"full10":None,"reduced7":"investigatory_7a_v1"}
...
def main() -> Dict[str, Any]:
    ...
    return {
        'study':'reduced_axis_same_location_near_location_predictive_v2',
        'sample_count': len(states),
        'near_location_compare': near_location_compare(states),
        'bandit_hidden_compare': bandit_predictive_compare(),
        'renewal_local_support_compare': renewal_predictive_compare(),
        'maze_topology_compare': maze_predictive_compare(),
    }
