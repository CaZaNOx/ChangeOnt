...
class UCB1Agent:
    def __init__(self, n_arms: int):
        self.n=n_arms; self.counts=[0]*n_arms; self.values=[0.0]*n_arms; self.t=0
    def select(self)->int:
        for a in range(self.n):
            if self.counts[a]==0: return a
        import math
        self.t+=1
        u=[ self.values[a] + (2.0*math.log(self.t)/self.counts[a])**0.5 for a in range(self.n) ]
        return max(range(self.n), key=lambda a: u[a])
    def update(self,a:int,r:float)->None:
        self.counts[a]+=1; n=self.counts[a]; q=self.values[a]; self.values[a]= q + (r-q)/n

class EpsilonGreedyAgent:
    def __init__(self,n_arms:int,epsilon:float=0.1,seed:int=0):
        from random import Random
        self.n=n_arms; self.eps=float(epsilon); self.counts=[0]*n_arms; self.values=[0.0]*n_arms; self.rng=Random(seed)
    def select(self)->int:
        if self.rng.random()<self.eps: return self.rng.randrange(self.n)
...
def _load_config(args: argparse.Namespace) -> BanditConfig:
    if args.config:
        ...
        env = data.get("env",{}); agent = data.get("agent",{}); out = data.get("out","outputs/bandit_ucb")
        return BanditConfig(
    ...
    return BanditConfig(probs=probs, horizon=int(args.horizon), agent=args.agent.lower(),
...
def main()->None:
    ...
    metrics_path=out_dir/"metrics.jsonl"; budget_path=out_dir/"budget.csv"; plot_path=out_dir/"quick_plot.png"

    env=BernoulliBanditEnv(cfg.probs, horizon=cfg.horizon); env.reset(seed=cfg.seed)
    ...
    if cfg.agent=="ucb1":
        ...
        from agents.co.agent_bandit import COBanditCfg, COBanditAgent  # type: ignore
        agent=COBanditAgent(COBanditCfg(n_arms=env.n_arms, seed=cfg.seed)); agent.reset()
    else:
        raise ValueError(f"unknown agent type: {cfg.agent}")

    best_mean=max(cfg.probs); cum_regret=0.0; pulls=[0]*env.n_arms; budget_rows=[]
    t=0; done=False
    while not done:
        a=agent.select()
        ...
        agent.update(a,r)
        pulls[a]+=1; t+=1
        cum_regret += (best_mean - cfg.probs[a])
    ...
    write_metric_line(metrics_path, {"metric":"pulls_summary","t":t,"pulls":pulls,
