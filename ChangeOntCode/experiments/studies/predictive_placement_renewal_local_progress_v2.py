...
def _core(progress_override=None):
    cfg={
        "name":"predictive_renewal_progress_v2",
...
def run_trial(noise, shape, seed, horizon=120):
    env=CodebookRenewalEnvW(EnvCfg(A=8,L_win=6,p_ren=0.02,p_noise=noise,T_max=horizon), seed=seed)
...
def main():
    variants={"clean":0.0,"noisy":0.2}
    shapes={"canonical":None,"progress_low":0.0,"progress_high":1.0}
    out={"study":"predictive_placement_renewal_local_progress_v2","family":"renewal","axis":"local_progress_reliability","warmup_schedule":[0,1,2,3,4,5],"results":{}}
    for vname, noise in variants.items():
        out["results"][vname]={}
        for sname, sval in shapes.items():
            trials=[run_trial(noise, sval, seed=s, horizon=120) for s in range(10)]
