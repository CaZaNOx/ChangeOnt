...
from experiments.studies._descriptor_plane_v4 import problem_contract_for_family
from experiments.studies._descriptor_plane_factorized_v1 import (
    POSTURES,
    bandit_factorized,
    renewal_factorized,
    descriptor_axes_from_factorized,
    predicted_order,
    posture_scores,
...
OUT = Path("outputs/stoa_compare_factorized_runtime_v1.json")
STUDY = "stoa_compare_factorized_runtime_v1"
...
def _co_params(spec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    factorized = bandit_factorized(spec["probs"], horizon=BANDIT_HORIZON) if family == "bandit" else renewal_factorized(p_ren=spec["p_ren"], p_noise=spec["p_noise"], horizon=RENEWAL_HORIZON, action_count=spec["A"])
    descriptor = descriptor_axes_from_factorized(factorized)
    pred_scores = posture_scores(factorized)
    pred = predicted_order(factorized)
    best_posture = pred[0]
    ...
    params["prediction_protocol"] = {
        ...
        "factorized_basis": dict(factorized),
        "status": "investigatory",
    }
