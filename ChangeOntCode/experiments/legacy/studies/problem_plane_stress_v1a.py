...
from experiments.studies._co_eval_common import DEFAULT_CANONICAL_AGENT_NAME, DEFAULT_CANONICAL_MANIFEST, assert_valid_co_rollout, build_validated_co_core, load_co_manifest_params
from experiments.studies._descriptor_plane_v4 import POSTURES, bandit_descriptor, renewal_descriptor, posture_scores, predicted_order, problem_contract_for_family, target_scope_for_family

OUT = Path('outputs/problem_plane_stress_v1a.json')
STUDY='problem_plane_stress_v1a'
...
def _params(spec, posture_name, descriptor, pred_scores, pred):
    ...
    p['descriptor_hypothesis']={"target_scope":target_scope_for_family(family),"axes":dict(descriptor),"status":"investigatory","source":STUDY}
    p['kernel_posture']={"name":posture_name,"axes":dict(POSTURES[posture_name]),"status":"investigatory"}
    p['prediction_protocol']={"base_problem":{"name":spec['name']},"predicted_ordering_before":list(pred),"predicted_scores":dict(pred_scores),"status":"investigatory"}
...
def _run_bandit(spec, posture, descriptor, pred_scores, pred, seed):
    ...
    best=max(spec['probs']); regret=0.0; votes=[]; policies=[]
    for t in range(BANDIT_HORIZON):
        sel=agent.select({'family':'bandit','t':t,'n_arms':env.n_arms})
        a=int(sel.get('action',0)); votes.append(int(sel.get('signal_bus_votes',0))); policies.append(str(sel.get('co_policy','bandit:safe_default')))
        _,r,done,_=env.step(a); agent.update({'action':a,'reward':float(r),'done':bool(done)})
...
def _run_renewal(spec, posture, descriptor, pred_scores, pred, seed):
    ...
    obs,_,done,_=env.reset(); agent=COAdapterRenewal(core=core); rewards=[]; votes=[]; policies=[]; t=0
    while not done and t<RENEWAL_HORIZON:
        sel=agent.select({'family':'renewal','obs':int(obs),'t':t,'A':int(spec['A']),'L_win':int(spec['L_win'])})
        a=int(sel.get('action',0)); votes.append(int(sel.get('signal_bus_votes',0))); policies.append(str(sel.get('co_policy','renewal:safe_default')))
        obs,r,done,_=env.step(a); agent.update({'observation':int(obs),'reward':float(r),'done':bool(done),'action':a}); rewards.append(float(r)); t+=1
    if policies and all(p=='renewal:safe_default' for p in policies) and all(v==0 for v in votes): raise RuntimeError('invalid renewal rollout')
...
def _eval(spec):
    ...
    scope=target_scope_for_family(family); pred_scores=posture_scores(descriptor,target_scope=scope); pred=predicted_order(descriptor,target_scope=scope)
    rec={}
    for posture in POSTURES:
        vals=[_run_bandit(spec, posture, descriptor, pred_scores, pred, s) if family=='bandit' else _run_renewal(spec, posture, descriptor, pred_scores, pred, s) for s in SEEDS]
    ...
    if family=='bandit':
        ...
        observed=[k for k,_ in sorted(((k,v['mean_score']) for k,v in rec.items()), key=lambda kv:(-kv[1],kv[0]))]
    return {"descriptor":descriptor,"predicted_order":pred,"observed_order":observed,"exact_match":pred==observed,"postures":rec}

def main():
    out={"study":STUDY,"status":"executed","seeds":SEEDS,"bandit_horizon":BANDIT_HORIZON,"renewal_horizon":RENEWAL_HORIZON,"bandit":{},"renewal":{}}
    for spec in BANDIT_TASKS: out['bandit'][spec['name']]=_eval(spec)
    for spec in RENEWAL_TASKS: out['renewal'][spec['name']]=_eval(spec)
