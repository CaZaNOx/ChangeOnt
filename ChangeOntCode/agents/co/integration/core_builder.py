...
def _instantiate_components(params: Dict[str, Any], classes: Dict[str, Dict[str, Any]]) -> tuple[Any, Dict[str, Any], List[Any], Dict[str, Any]]:
    """
    Returns: (header, primitives, elements, combinators)
    """
    # -------- Header --------
    hcfg  = dict(params.get("header", {}))
    # allow 'type' or 'mode' to select header class; default 'SSI'
    htype = str(hcfg.pop("type", hcfg.pop("mode", "SSI")))
    header_cls = classes.get("headers", {}).get(htype)
    if header_cls is None:
        # fallback: make a tiny header that just surfaces mode & (optional) t
        class _Header:
            ...
            def update(self, observation: Dict[str, Any]) -> Dict[str, Any]:
                ...
                # accept several common names; runner currently uses "step"
                t = (observation.get("t", None)
                    if "t" in observation
                    else observation.get("step", None))
                if fam is not None:
                ...
                if t is not None:
                    try:
                        ...
                        pass
                return {
                    "header_mode": self.mode,
                    "t": self.state.get("t", None),
                    "family": fam
                }
    ...
    # Instantiate only requested primitives; if none provided, bring sensible defaults if available
    if prim_cfg:
        ...
        # optional defaults (best-effort)
        for name in ("visit_tracker", "bandit_stats", "ngram_model"):
            cls = prim_classes.get(name)
            if cls:
                primitives[name] = cls()

    # -------- Elements (respect YAML/insertion order; force action_head last) --------
    el_cfg_map = dict(params.get("elements", {}))  # preserves insertion order from loader
    element_classes = classes.get("elements", {})

    # Collect enabled entries by insertion order
    names = [k for k, v in el_cfg_map.items() if not isinstance(v, dict) or v.get("enabled", True)]

    # Ensure action_head exists; if not present, add a default entry
    if "action_head" not in names and "action_head" in element_classes:
        el_cfg_map["action_head"] = {"enabled": True}

    # Move action_head to the end deterministically
    ...
    for name in names:
        ...
        if cls is None:  # unknown element name; skip
            continue
        kwargs = {}
        ...
        if isinstance(raw, dict):
            ...
            kwargs.pop("enabled", None)
        # Instantiate
        # Instantiate (filter kwargs to what the class actually accepts)
        import inspect
        ...
        try:
            ...
            # if we can't introspect, use best-effort
            valid_kwargs = kwargs
        ...
        try:
            ...
            raise TypeError(
                f"Element '{name}' could not be constructed with kwargs={valid_kwargs} (raw={kwargs}): {e}"
            ) from e
    ...
    # Prefer registry pipeline, else fallback C_Pipeline
    pipeline_cls = comb_classes.get("pipeline", C_Pipeline)
    order = list(params.get("combinator", {}).get("order", []))
    pipeline = pipeline_cls(order=order) if "order" in pipeline_cls.__init__.__code__.co_varnames else pipeline_cls()
...
def build_co_core(params: Dict[str, Any] | None = None) -> COAgentCore:
    ...
    Params shape (examples):
      ...
      primitives:
        ...
        ngram_model: {}
      combinator:
        order: []    # optional explicit element order (names must match registry keys)
      name: "CO_core"
    """
    cfg = dict(params or {})
