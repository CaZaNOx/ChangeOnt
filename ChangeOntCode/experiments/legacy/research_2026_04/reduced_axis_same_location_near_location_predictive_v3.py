...
def _core(family: str, *, reduction_mode: str | None = None, axis_override: Dict[str, float] | None = None):
    cfg={
        "name": f"reduction_cmp_v3_{family}_{reduction_mode or '10a'}",
...
def sample_states() -> List[Dict[str, Any]]:
        ...
        for seed in range(2):
            ...
            for t in range(6):
                ...
                if packet:
                    ...
                    out.append({'family':'bandit','variant':label,'seed':seed,'t':t,'axes10':bundle['effective_axes'],'axes7':project_full_to_reduced7(bundle['effective_axes'])})
    ...
    for label, noise, p_ren in [("renewal_stable_noisy",0.05,0.02), ("renewal_volatile_noisy",0.20,0.12)]:
        ...
        while not done and t < 6:
            sel=ag.select({'family':'renewal','t':t,'A':4,'obs':obs})
            packet=dict(ag._last_obs or {})
            if packet:
                bundle=_bundle(core, packet)
                out.append({'family':'renewal','variant':label,'seed':seed,'t':t,'axes10':bundle['effective_axes'],'axes7':project_full_to_reduced7(bundle['effective_axes'])})
            a=sel.get('action',0); obs,r,done,_=env.step(a); ag.update({'action':a,'reward':float(r),'done':bool(done),'obs':obs,'A':4}); t += 1
    # maze
    for label, spec_fn in [("maze_visible_static", lambda s: MazeSpec(width=7,height=7,seed=s,partial_observability=False,dynamic_walls=False)), ("maze_partial_dyn", lambda s: MazeSpec(width=7,height=7,seed=s,partial_observability=True,view_radius=1,dynamic_walls=True,wall_flip_prob=0.08,max_flips_per_step=1))]:
        ...
        for t in range(6):
            ...
            if packet:
                bundle=_bundle(core, packet)
                out.append({'family':'maze','variant':label,'seed':seed,'t':t,'axes10':bundle['effective_axes'],'axes7':project_full_to_reduced7(bundle['effective_axes'])})
...
def near_location(states: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    out={}
    for mode,key in [('full10','axes10'),('reduced7','axes7')]:
        out[mode]={}
        for a,b in pairs:
...
def renewal_compare() -> Dict[str, Any]:
    variants={'stable_noisy': {'noise':0.2,'p_ren':0.02}, 'volatile_noisy': {'noise':0.2,'p_ren':0.12}}
    modes={"full10":None,"reduced7":"investigatory_7a_v1"}
    ...
    for mode_name, reduction_mode in modes.items():
        ...
        for vname, cfg in variants.items():
            ...
            for sname, sval in shapes.items():
                ...
                env=CodebookRenewalEnvW(EnvCfg(A=8,L_win=6,p_ren=cfg['p_ren'],p_noise=cfg['noise'],T_max=30), seed=0)
                ...
                out[mode_name][vname][sname]={'mean_reward_rate':mean(rewards)}
    return out


def maze_compare() -> Dict[str, Any]:
    variants={'visible_static': lambda s: MazeSpec(width=7,height=7,seed=s,partial_observability=False,dynamic_walls=False), 'partial_dynamic': lambda s: MazeSpec(width=7,height=7,seed=s,partial_observability=True,view_radius=1,dynamic_walls=True,wall_flip_prob=0.08,max_flips_per_step=1)}
    modes={"full10":None,"reduced7":"investigatory_7a_v1"}
    ...
    for mode_name, reduction_mode in modes.items():
        ...
        for vname, spec_fn in variants.items():
            ...
            for sname, sval in shapes.items():
                ...
                env=GridMazeEnv(spec=spec_fn(0)); env.reset(seed=0); core=_core('maze', reduction_mode=reduction_mode, axis_override=override); ag=COAdapterMaze(core=core)
                ...
                out[mode_name][vname][sname]={'solve_rate':1.0 if solved else 0.0,'mean_steps':steps}
    return out
...
def main() -> Dict[str, Any]:
    ...
    return {
        'study':'reduced_axis_same_location_near_location_predictive_v3',
