...
OUT = Path("outputs/problem_plane_geometry_refactor_v1.json")
...
def _params_for(spec: Dict[str, Any], posture_name: str, descriptor: Dict[str, float], pred: List[str]) -> Dict[str, Any]:
    ...
    params["descriptor_hypothesis"] = {
        ...
        "source": "problem_plane_geometry_refactor_v1",
    }
...
def _run_bandit(spec: Dict[str, Any], posture_name: str, descriptor: Dict[str, float], pred: List[str], seed: int) -> Dict[str, Any]:
    core = build_validated_co_core(_params_for(spec, posture_name, descriptor, pred), study_name="problem_plane_geometry_refactor_v1", manifest_path=DEFAULT_CANONICAL_MANIFEST, agent_name=DEFAULT_CANONICAL_AGENT_NAME)
    ...
    for t in range(BANDIT_HORIZON):
        ...
        if done:
            break
    assert_valid_co_rollout(study_name="problem_plane_geometry_refactor_v1", signal_bus_votes=votes, co_policies=policies)
    return {
        "final_regret": regret,
...
def _run_renewal(spec: Dict[str, Any], posture_name: str, descriptor: Dict[str, float], pred: List[str], seed: int) -> Dict[str, Any]:
    core = build_validated_co_core(_params_for(spec, posture_name, descriptor, pred), study_name="problem_plane_geometry_refactor_v1", manifest_path=DEFAULT_CANONICAL_MANIFEST, agent_name=DEFAULT_CANONICAL_AGENT_NAME)
...
def _env_summary(spec: Dict[str, Any], family: str) -> Dict[str, Any]:
    descriptor = bandit_descriptor(spec["probs"]) if family == "bandit" else renewal_descriptor(p_ren=spec["p_ren"], p_noise=spec["p_noise"])
    pred = predicted_order(descriptor)
    runs = {}
    for posture_name in POSTURES:
        posture_runs = []
        for seed in SEEDS:
            if family == "bandit":
                posture_runs.append(_run_bandit(spec, posture_name, descriptor, pred, seed))
            else:
                posture_runs.append(_run_renewal(spec, posture_name, descriptor, pred, seed))
        if family == "bandit":
            score = mean(r["final_regret"] for r in posture_runs)
        else:
            score = mean(r["mean_reward"] for r in posture_runs)
        runs[posture_name] = {
            "mean_score": score,
            "seed_runs": posture_runs,
        }
    if family == "bandit":
        observed = [k for k, _ in sorted(((k, v["mean_score"]) for k, v in runs.items()), key=lambda kv: kv[1])]
    else:
        observed = [k for k, _ in sorted(((k, v["mean_score"]) for k, v in runs.items()), key=lambda kv: (-kv[1], kv[0]))]
    return {
...
def main() -> None:
    out = {"study": "problem_plane_geometry_refactor_v1", "bandit": {}, "renewal": {}}
    for spec in BANDITS:
